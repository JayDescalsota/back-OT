...
func (ec *executionContext) resolveEntity(
	...
	case "Goal":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Goal": %w`, err)
		}
		switch resolverName {
		...
		case "findGoalByID":
			...
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findGoalByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindGoalByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Goal": %w`, err)
			}

			return entity, nil
		}
	case "Guardian":
		resolverName, err := entityResolverNameForGuardian(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Guardian": %w`, err)
		}
		switch resolverName {
		...
		case "findGuardianByID":
			id0, err := ec.unmarshalNID2string(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findGuardianByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindGuardianByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Guardian": %w`, err)
			}
	...
	case "Patient":
		...
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Patient": %w`, err)
		}
		switch resolverName {
		...
		case "findPatientByID":
			id0, err := ec.unmarshalNID2string(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findPatientByID(): %w`, err)
			}
			entity, err := ec.Resolvers.Entity().FindPatientByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Patient": %w`, err)
			}
...
func entityResolverNameForGoal(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Goal", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Goal", ErrTypeNotFound))
			break
		}
		return "findGoalByID", nil
	}
	return "", fmt.Errorf("%w for Goal due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForGuardian(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Guardian", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Guardian", ErrTypeNotFound))
			break
		}
		return "findGuardianByID", nil
	}
	return "", fmt.Errorf("%w for Guardian due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForPatient(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"id\" for Patient", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Patient", ErrTypeNotFound))
			break
		}
		return "findPatientByID", nil
	}
	return "", fmt.Errorf("%w for Patient due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
