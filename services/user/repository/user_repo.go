...
func (r *UserRepo) UpsertProfile(ctx context.Context, profile *models.UserProfile) error {
	_, err := r.db.NewInsert(profile).
		On("CONFLICT (user_id) DO UPDATE SET first_name = EXCLUDED.first_name, last_name = EXCLUDED.last_name, middle_name = EXCLUDED.middle_name, suffix = EXCLUDED.suffix, title = EXCLUDED.title, phone = EXCLUDED.phone, mobile = EXCLUDED.mobile, date_of_birth = EXCLUDED.date_of_birth, gender = EXCLUDED.gender, address_id = EXCLUDED.address_id, timezone = EXCLUDED.timezone, preferred_language = EXCLUDED.preferred_language, emergency_contact_name = EXCLUDED.emergency_contact_name, emergency_contact_phone = EXCLUDED.emergency_contact_phone, emergency_contact_relation = EXCLUDED.emergency_contact_relation, notes = EXCLUDED.notes, updated_at = EXCLUDED.updated_at, updated_by = EXCLUDED.updated_by, updated_action = EXCLUDED.updated_action").
		Exec(ctx)
	return err
}
